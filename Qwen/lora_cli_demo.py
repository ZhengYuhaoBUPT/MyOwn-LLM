...
def _load_model_tokenizer(args):
    ...
    # === 添加：加载 LoRA adapter ===
    lora_path = "output_qwen"  # 你可以绝对路径也可以相对路径
    if os.path.exists(os.path.join(lora_path, "adapter_model.safetensors")):
        print(f"[INFO] Loading LoRA adapter from {lora_path}")
        base_model = PeftModel.from_pretrained(base_model, lora_path, device_map=device_map)
        base_model = base_model.merge_and_unload()
        print("[INFO] LoRA adapter merged into base model.")
    else:
        print(f"[WARNING] LoRA adapter not found at {lora_path}, using base model only.")
